...
export const getUser = async (fid: number): Promise<User | null> => {
	try {
		const user = db.select().from(schema.user).where(eq(schema.user.fid, fid)).limit(1).get();
		if (user) {
			return user;
		}
		return null;
...
export const deleteUserNotificationDetails = async (fid: number): Promise<void> => {
	try {
		db.update(schema.user).set({ notificationDetails: null }).where(eq(schema.user.fid, fid)).run();
	} catch (error) {
		console.error(error);
	}
};
