...
import {FarkleRoomImpl} from '../src/impl/FarkleRoomImpl.sol';
...
contract DeployFarkleGameBeacon is Script {
	...
	function run() external {
		...
		FarkleRoomImpl implementation = new FarkleRoomImpl();
		console.log('FarkleRoomImpl deployed at:', address(implementation));
	...
	function deployBeacon(
		...
		require(initialImplementation.code.length > 0, 'Implementation must have code');
	...
	function salt() internal view returns (bytes32) {
		...
		return keccak256(abi.encodePacked(block.chainid, msg.sender, 'FarkleRoomBeacon'));
	}
}
