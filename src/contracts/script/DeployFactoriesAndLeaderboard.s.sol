...
contract DeploySystem is Script {
	function run(address gameBeaconAddress, address roomBeaconAddress) external {
		require(gameBeaconAddress != address(0), 'Game beacon address cannot be zero');
		require(roomBeaconAddress != address(0), 'Room beacon address cannot be zero');

		vm.startBroadcast();

		// 1. Deploy the FarkleGameFactory using the provided game beacon address
		FarkleGameFactory gameFactory = new FarkleGameFactory(gameBeaconAddress);
		...
		console.log('Deploying FarkleLeaderboard impl at:', address(leaderboardImpl));
		...
		console.log('Deploying FarkleLeaderboardProxy at:', address(leaderboardProxy));
